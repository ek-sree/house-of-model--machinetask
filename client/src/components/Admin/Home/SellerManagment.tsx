...
import { adminAxios, createAdminAxios } from '../../../constraints/axios/adminAxios';
...
const SellerManagment = () => {
    ...
    async function fetchSellers(page: number){
        try {
            const response = await adminAxios.get(`${adminEndpoints.fetchSellers}?page=${page}&limit=${limit}`)
            ...
            console.log("Error occured while fetching sellers",error);
    ...
    const blockUser = async (userId: string) => {
        try {
            const response = await adminAxios.put(`${adminEndpoints.blockSeller}/${userId}`);
            if (response.status == 200) {
                setUsers((prevUsers) =>
                    prevUsers.map((user) =>
  ...
  return (
    ...
    <div className="flex justify-end mr-28 mt-5">
        <input type="text" value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)} name="search" id="" placeholder="search user" className="px-3 py-2 rounded-lg bg-slate-300 text-white font-semibold"/>
